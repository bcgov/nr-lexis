...
Raw XML may use the legacy bean namespace or an unqualified root. Lower-camel requests use `item`
array elements, while the .NET PascalCase form generated by NEXCOL uses `string` elements. The raw
XML response mirrors the field casing and array-element name from the request. For example:
...
or schema-version-3 `LexisSubmission`. The inner `LexisSubmission` is also accepted as raw XML.
...
LEXIS selects the bundled version-2 or version-3 legacy XSD from the declared
