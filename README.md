...
TEST IDIR credentials and fixture identifiers from GitHub `test` environment secrets and runs
Playwright with a reporter that suppresses sensitive output. Business BCeID browser regression is
intentionally not scheduled because repeated automated login attempts can lock the TEST account. See
...
- [docs/legacy-bug-register.md](docs/legacy-bug-register.md) - Legacy defects, modern corrections and proposed procedure changes deferred pending a business request.
...
- [docs/rtm-amv-ui-and-procedure-contract.md](docs/rtm-amv-ui-and-procedure-contract.md) - RTM AMV unified-table rules and atomic Oracle persistence contract.
